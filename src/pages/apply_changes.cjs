...
function applyChanges() {
  ...
  // 1. Delete old unused variables/functions to avoid TS errors
  ...
  // 2. Add handlers for nested gastos
  const handlersStr = `
...
`;
  ...
  // 4. Update gastosToUse calculation
...
`;
  ...
  // 5. Build the UI!
  const blockStart = '{/* Desglose de Gastos */}';
  const blockEnd = '{/* Hidden PDF Template */}';
  const iStart = content.indexOf(blockStart);
  const iEnd = content.indexOf(blockEnd);
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4 border-b border-gray-100 pb-6">
                  ...
                  <div className="flex flex-wrap gap-2 w-full sm:w-auto">
                    ...
                    <button onClick={handleExportPDF} disabled={isExporting} className="flex flex-1 sm:flex-none items-center justify-center gap-2 text-sm bg-slate-800 text-white border border-slate-700 px-4 py-2.5 rounded-xl font-bold shadow-md hover:bg-slate-700 transition-all hover:-translate-y-0.5">
                      ...
                      {isExporting ? 'Generar PDF' : 'Generar PDF'}
                    </button>
                  </div>
                </div>
...
`;
    content = content.substring(0, iStart) + leftColContent + '\n            ' + content.substring(iEnd);
  }

  // Final fix: Because the above creates <div className="space-y-6"> and then closes the left col, 
  // Let's ensure the grid syntax closing is handled properly since it replaces old Desglose until Hidden PDF Template 
  // Wait, old structure inside grid was: <div col-span-2> ... desglose ... </div> <div col-span-1> metrics </div> </div>
  // My replacement above ends with `</div> </div> </div>`, which perfectly matches: closes right col, closes grid!
