...
export default function PartnerDetalle() {
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col max-w-2xl mx-auto bg-white shadow-xl overflow-x-hidden text-slate-900 pb-12">
      {/* TopAppBar */}
      <div className="sticky top-0 z-50 flex items-center bg-white/90 backdrop-blur-md p-4 justify-between border-b border-slate-200 shadow-sm">
        <button 
          onClick={() => navigate(-1)}
          className="text-slate-600 flex size-10 shrink-0 items-center justify-center rounded-full hover:bg-slate-100 transition-colors"
        >
          <ArrowLeft size={24} />
        </button>
        <h2 className="text-lg font-bold leading-tight tracking-tight flex-1 text-center text-slate-800">
          Novedad de Socio
        </h2>
        <button className="text-slate-600 flex size-10 items-center justify-center rounded-full hover:bg-slate-100 transition-colors">
          <Share2 size={24} />
        </button>
      </div>
      ...
      <div className="p-6 md:p-8">
        ...
        <div 
          className="prose prose-slate max-w-none prose-p:text-slate-600 prose-p:leading-relaxed prose-headings:text-slate-900 prose-headings:font-bold prose-a:text-[#3A5F4B] hover:prose-a:text-[#2d4a3a] prose-strong:text-slate-900 prose-ul:list-disc prose-ul:pl-5 prose-li:text-slate-600 prose-li:marker:text-[#3A5F4B]"
